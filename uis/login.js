...
document.getElementById('login-form').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const 회원가입_페이지_URL = '../uis/signup.html'; // ⚠️ 회원가입 페이지 파일명 (실제 파일명으로 수정!)
      const 비밀번호_재설정_페이지_URL = '../uis/find_password.html'; // ⚠️ 비밀번호 재설정 페이지 파일명 (실제 파일명으로 수정!)
      ...
      if (
        ...
        window.location.href = 'index.html'; // ⚠️ 메인 페이지 경로 (실제 경로로 수정!)
      } else {
